...
jepsen_dcs_majority_kill_services="${PACMAN_JEPSEN_DCS_MAJORITY_KILL_SERVICES:-pacman-dcs-2 pacman-dcs-3}"
jepsen_dcs_majority_partition_services="${PACMAN_JEPSEN_DCS_MAJORITY_PARTITION_SERVICES:-pacman-dcs-2 pacman-dcs-3}"
jepsen_dcs_restart_services="${PACMAN_JEPSEN_DCS_RESTART_SERVICES:-pacman-dcs pacman-dcs-2 pacman-dcs-3}"
jepsen_dcs_slow_services="${PACMAN_JEPSEN_DCS_SLOW_SERVICES:-pacman-dcs pacman-dcs-2 pacman-dcs-3}"
jepsen_dcs_slow_min_latency_ms="${PACMAN_JEPSEN_DCS_SLOW_MIN_LATENCY_MS:-100}"
jepsen_smoke_cases_default="append-smoke:none"
jepsen_nightly_cases_default="append-smoke:none append-switchover:switchover append-failover:kill append-failover:packet append-failover:packet,kill append-failover:primary-dcs-partition append-failover:primary-replication-partition append-failover:failover-chain open-transaction-failover:kill vip-routing:switchover append-dcs-quorum:dcs-kill-one append-dcs-quorum:dcs-lose-majority append-dcs-quorum:primary-dcs-majority-partition append-dcs-quorum:dcs-full-restart append-dcs-quorum:dcs-slow-network single-key-register:packet read-committed-txn:slow-network serializable-txn:packet,kill append-failover:repeated-failure"
...
append-dcs-quorum-dcs-lose-majority append-dcs-quorum:dcs-lose-majority Append workload while killing two etcd DCS members.
append-dcs-quorum-primary-dcs-majority-partition append-dcs-quorum:primary-dcs-majority-partition Append workload while isolating the current primary from a DCS majority.
append-dcs-quorum-dcs-full-restart append-dcs-quorum:dcs-full-restart Append workload while restarting all etcd DCS members.
append-dcs-quorum-dcs-slow-network append-dcs-quorum:dcs-slow-network Append workload while adding latency to all etcd DCS members.
...
resolve_jepsen_case_spec() {
  ...
  case "${name}" in
    ...
    append-dcs-quorum-dcs-lose-majority | append-dcs-quorum:dcs-lose-majority) printf 'append-dcs-quorum:dcs-lose-majority\n' ;;
    append-dcs-quorum-primary-dcs-majority-partition | append-dcs-quorum:primary-dcs-majority-partition) printf 'append-dcs-quorum:primary-dcs-majority-partition\n' ;;
    append-dcs-quorum-dcs-full-restart | append-dcs-quorum:dcs-full-restart) printf 'append-dcs-quorum:dcs-full-restart\n' ;;
    append-dcs-quorum-dcs-slow-network | append-dcs-quorum:dcs-slow-network) printf 'append-dcs-quorum:dcs-slow-network\n' ;;
