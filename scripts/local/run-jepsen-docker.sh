...
Environment:
  ...
  PACMAN_JEPSEN_WORKLOAD_CLIENTS   workload client count passed to the harness
  PACMAN_JEPSEN_WORKLOAD_KEYS      workload key count passed to the harness
  PACMAN_JEPSEN_DCS_KILL_SERVICE   target service for dcs-kill-one
  PACMAN_JEPSEN_DCS_MAJORITY_KILL_SERVICES target services for dcs-lose-majority
  PACMAN_JEPSEN_DCS_MAJORITY_PARTITION_SERVICES target DCS services isolated from primary
  PACMAN_JEPSEN_DCS_RESTART_SERVICES target services for dcs-full-restart
  PACMAN_JEPSEN_DCS_SLOW_SERVICES target services for dcs-slow-network
...
docker_args=(
  ...
  -e "PACMAN_JEPSEN_WORKLOAD_CLIENTS=${PACMAN_JEPSEN_WORKLOAD_CLIENTS:-}"
  -e "PACMAN_JEPSEN_WORKLOAD_KEYS=${PACMAN_JEPSEN_WORKLOAD_KEYS:-}"
  -e "PACMAN_JEPSEN_NEMESIS_HOLD_SECONDS=${PACMAN_JEPSEN_NEMESIS_HOLD_SECONDS:-}"
  -e "PACMAN_JEPSEN_POST_NEMESIS_SETTLE_SECONDS=${PACMAN_JEPSEN_POST_NEMESIS_SETTLE_SECONDS:-}"
  -e "PACMAN_JEPSEN_APPEND_SWITCHOVER_OP_DELAY_SECONDS=${PACMAN_JEPSEN_APPEND_SWITCHOVER_OP_DELAY_SECONDS:-}"
  -e "PACMAN_JEPSEN_DCS_KILL_SERVICE=${PACMAN_JEPSEN_DCS_KILL_SERVICE:-}"
  -e "PACMAN_JEPSEN_DCS_MAJORITY_KILL_SERVICES=${PACMAN_JEPSEN_DCS_MAJORITY_KILL_SERVICES:-}"
  -e "PACMAN_JEPSEN_DCS_MAJORITY_PARTITION_SERVICES=${PACMAN_JEPSEN_DCS_MAJORITY_PARTITION_SERVICES:-}"
  -e "PACMAN_JEPSEN_DCS_RESTART_SERVICES=${PACMAN_JEPSEN_DCS_RESTART_SERVICES:-}"
  -e "PACMAN_JEPSEN_DCS_SLOW_SERVICES=${PACMAN_JEPSEN_DCS_SLOW_SERVICES:-}"
