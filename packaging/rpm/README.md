...
This directory contains the RPM packaging entrypoint for PACMAN on EL9
compatible systems such as Rocky Linux, AlmaLinux, and RHEL. The packages are
assembled by the Go automation in [`tools/rpmctl`](../../tools/rpmctl) with
nFPM from staged build artifacts rather than `rpmbuild`.
...
- runs `go run ./tools/rpmctl` inside the container
- copies the workspace into a writable build directory from Go
- builds the Go binaries and PostgreSQL extension from Go orchestration
- runs `nfpm package --packager rpm` for the runtime and extension RPMs
...
The default config points to `/etc/pacman/admin-token`, and the post-install
lifecycle hook generates that token file on first install when it is absent.

## Upgrade-Safe Config Policy

The shipped config files are marked as nFPM `config|noreplace` entries:
...
The admin bearer token file is owned as an RPM ghost path. The post-install
script generates `/etc/pacman/admin-token` only on first install if it does not
already exist, then preserves it across upgrades.

## Lifecycle Hooks

The main `pacman` RPM now includes package lifecycle handling for:

- creating a dedicated `pacman` system user and group before install
...
- reloading systemd unit metadata when `systemctl` is available
