...
INTEGRATION_GROUP_PATRONI := ^(TestPatroniInspiredCloudRestoreCommandWithRenderedStandbyInTestcontainers|TestPatroniInspiredBackupRestoreCloudNegativeCasesInTestcontainers|TestPatroniProbeCompatibilityWithContainerFixture|TestPatroniMonitoringDocumentsWithContainerFixture|TestPatroniAdminCompatibilityWithContainerFixture|TestPatroniInspiredWatchdogAndFencingInTestcontainers|TestPatroniMigrationRejectsEtcdHostWithoutScheme|TestPatroniMigrationRejectsUnsupportedDCSBackend|TestPatroniMigrationRejectsUnsupportedExhibitorAndKubernetesConfigs|TestPatroniMigrationRejectsMissingClusterName|TestPatroniMigratedSingleNodeBootstrapsAndServesHealth|TestPatroniMigratedClusterNameMatchesPatroniScope|TestPatroniMigratedMembershipExamplesPreserveInitialPrimaryAndExpectedMembers|TestPatroniMigratedPrimaryProbeWorks|TestPatroniMigratedMetricsEndpointServesClusterMetrics|TestPatroniMigratedNode2RESTAuthVariantRequiresBearerToken)$$
...
INTEGRATION_GROUP_DCS_CONFORMANCE := ^(TestEtcdDCSConformanceInRunner|TestRaftThreeNodeReplicationAndWatch|TestRaftThreeNodeLeaderFailover|TestPatroniDCSBackendContractsInTestcontainers)$$
...
test-dcs-conformance: docker-build-test-image docker-build-pgext-image
